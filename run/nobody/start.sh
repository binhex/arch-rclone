...
while true; do
	...
	for rclone_media_shares_item in "${rclone_media_shares_list[@]}"; do
		...
		if [[ "${ENABLE_WEBUI}" == 'yes' ]]; then
			...
			/usr/bin/rclone rc "sync/${RCLONE_OPERATION}" srcFs="/media/${rclone_media_shares_item}" dstFs="${RCLONE_REMOTE_NAME}:/${rclone_media_shares_item}" --config="${RCLONE_CONFIG_PATH}" ${RCLONE_USER_FLAGS} --timeout=0 --rc-user=${WEBUI_USER} --rc-pass=${WEBUI_PASS} --log-file="${rclone_log}" --log-level INFO
		else
			/usr/bin/rclone "${RCLONE_OPERATION}" "/media/${rclone_media_shares_item}" "${RCLONE_REMOTE_NAME}:/${rclone_media_shares_item}" --config="${RCLONE_CONFIG_PATH}" ${RCLONE_USER_FLAGS} --log-file="${rclone_log}" --log-level INFO
		fi
